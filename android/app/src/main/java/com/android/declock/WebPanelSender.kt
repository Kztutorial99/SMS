...
object WebPanelSender {
    ...
    fun send(context: Context, packageName: String, appLabel: String, title: String, text: String) {
        val base = BuildConfig.PANEL_API_URL.trimEnd('/')
        val token = BuildConfig.PANEL_DEVICE_TOKEN
        if (base.isBlank() || token.isBlank()) { Log.w("WebPanelSender", "Panel API not configured"); return }
        val body = JSONObject().apply {
            put("deviceId", deviceId(context)); put("deviceLabel", DeviceInfo.deviceLabel())
            put("android", DeviceInfo.androidVersion()); put("sims", DeviceInfo.formatSims(context))
            put("packageName", packageName); put("appLabel", appLabel); put("title", title); put("text", text)
        }.toString()
        scope.launch {
            try {
                val request = Request.Builder().url("$base/api/device/event").header("x-device-token", token).post(body.toRequestBody("application/json".toMediaType())).build()
                client.newCall(request).execute().use { r -> Log.d("WebPanelSender", "Event response=${r.code}") }
            } catch (e: Exception) { Log.e("WebPanelSender", "Failed to send event", e) }
        }
    }
    ...
    fun heartbeat(context: Context) {
        ...
        scope.launch {
            try { val req = Request.Builder().url("$base/api/device/heartbeat").header("x-device-token", token).post(body.toRequestBody("application/json".toMediaType())).build(); client.newCall(req).execute().close() } catch (e: Exception) { Log.e("WebPanelSender", "Heartbeat failed", e) }
        }
    }
        ...
        scope.launch {
            try {
                ...
                client.newCall(req).execute().use { r ->
                    ...
                    val out = mutableListOf<JSONObject>(); for (i in 0 until arr.length()) out += JSONObject(arr.getString(i)); onCommands(out)
                }
